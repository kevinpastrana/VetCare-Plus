/**
 * FORMSCONSULTA.JS - JavaScript para formularios de Consultas
...
function validarCampo(campo) {
    ...
    // Validación de fecha de consulta (no puede ser futura)
    if (campo.id === 'fecha_consulta' && valor) {
        const fechaConsulta = new Date(valor);
        const hoy = new Date();
        hoy.setHours(0, 0, 0, 0);
        
        if (fechaConsulta > hoy) {
            mostrarError(campo, 'La fecha de consulta no puede ser futura');
            return false;
        }
    }
    
    // Validación de próxima cita (debe ser igual o posterior a hoy)
    if (campo.id === 'proxima_cita' && valor) {
        const proximaCita = new Date(valor);
        const hoy = new Date();
        hoy.setHours(0, 0, 0, 0);
        
        if (proximaCita < hoy) {
            mostrarError(campo, 'La próxima cita debe ser igual o posterior a hoy');
            return false;
        }
    }
...
async function enviarFormulario() {
    ...
    try {
        ...
        if (data.success) {
            mostrarMensaje('¡Consulta registrada exitosamente!', 'success');
            form.reset();
...
console.log('✅ Validación de formularios de Consultas cargada');