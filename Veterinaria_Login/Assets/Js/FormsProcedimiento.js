/**
 * FORMSPROCEDIMIENTO.JS - JavaScript para formularios de Procedimientos
...
function validarCampo(campo) {
    ...
    // Validación de fecha de procedimiento (no puede ser futura)
    if (campo.id === 'fecha_procedimiento' && valor) {
        const fechaProcedimiento = new Date(valor);
        const hoy = new Date();
        hoy.setHours(0, 0, 0, 0);
        
        if (fechaProcedimiento > hoy) {
            mostrarError(campo, 'La fecha del procedimiento no puede ser futura');
            return false;
        }
    }
    
    // Validación de hora de fin (debe ser posterior a hora de inicio)
    if (campo.id === 'hora_fin' && valor) {
        const horaInicio = document.getElementById('hora_inicio').value;
        if (horaInicio && valor <= horaInicio) {
            mostrarError(campo, 'La hora de fin debe ser posterior a la hora de inicio');
            return false;
        }
    }
    
    // Validación de costo
    if (campo.id === 'costo' && valor) {
        const costo = parseFloat(valor);
        if (isNaN(costo) || costo < 0) {
            mostrarError(campo, 'El costo debe ser un valor positivo');
            return false;
        }
    }
...
async function enviarFormulario() {
    ...
    console.log('Enviando procedimiento...'); // Debug
    ...
    try {
        ...
        if (data.success) {
            mostrarMensaje('¡Procedimiento registrado exitosamente!', 'success');
            form.reset();
            ...
            mostrarMensaje(data.message || 'Error al registrar el procedimiento', 'error');
...
// Formato de costo (números y punto decimal)
...
// Capitalizar nombre del procedimiento
...
console.log('✅ Validación de formularios de Procedimientos cargada');