/**
 * FORMSCARGO.JS - JavaScript para formularios de Cargos
...
function validarCampo(campo) {
    ...
    // Validación de longitud del nombre del cargo
    if (campo.id === 'nombre_cargo' && valor) {
        if (valor.length < 3) {
    ...
    if (campo.id === 'descripcion' && valor) {
        if (valor.length > 200) {
            mostrarError(campo, 'La descripción no puede exceder 200 caracteres');
            return false;
        }
    }
...
async function enviarFormulario() {
    ...
    console.log('Enviando cargo...'); // Debug
    ...
    try {
        ...
        if (data.success) {
            ...
            mostrarMensaje(data.message || 'Error al registrar el cargo', 'error');
...
// Capitalizar nombre del cargo
const nombreCargoInput = document.getElementById('nombre_cargo');
if (nombreCargoInput) {
    nombreCargoInput.addEventListener('blur', function() {
    ...
    // Prevenir caracteres especiales excepto espacios y guiones
    nombreCargoInput.addEventListener('input', function(e) {
...
// Mostrar preview del cargo mientras se escribe
const nombreInput = document.getElementById('nombre_cargo');
const descripcionTextarea = document.getElementById('descripcion');
const estadoSelect = document.getElementById('estado');

function actualizarPreview() {
    // Aquí podrías agregar un preview en tiempo real si lo deseas
...
console.log('✅ Validación de formularios de Cargos cargada');